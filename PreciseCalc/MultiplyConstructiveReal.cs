...
internal sealed class MultiplyConstructiveReal : ConstructiveReal
{
    ...
    {
        var halfPrec = (precision >> 1) - 1;
        var msdOp1 = _op1.GetMsd(halfPrec);
        int msdOp2;
        ...
        var prec2 = precision - msdOp1 - 3; // Precision needed for op2.
        ...
        // another 1/2 ulp.
        var appr2 = _op2.GetApproximation(prec2);
        if (appr2.Sign == 0) return Big0;

        msdOp2 = _op2.GetKnownMsd();
        var prec1 = precision - msdOp2 - 3; // Precision needed for op1.
        var appr1 = _op1.GetApproximation(prec1);

        var scaleDigits = prec1 + prec2 - precision;
