    ...
    IDivisionOperators<UnifiedReal, UnifiedReal, UnifiedReal>
{
    ...
    private const int BitLimit = 2_000_000;

    // Number of extra bits used in ToStringTruncated evaluation to prefer truncation to
    // rounding. Must be <= 30.
    private const int ExtraPrecision = 10;

    private const int CommonPowerLengthLimit = 200;
    ...
    private static readonly BoundedRational Br1Over1024 = new(1, 1024);
    ...
    {
        if (!arg.HasValue) return BoundedRational.Null;

        // Avoid real computations if we can. Only needed for performance.
        if (arg.CompareTo(BoundedRational.MinusHalf) >= 0 && arg.CompareTo(Br32) < 0) return arg;

        var argPlusHalf = arg + BoundedRational.Half;
        if (!argPlusHalf.HasValue) return BoundedRational.Null;

        var argPhFloor = argPlusHalf.Floor();
        var resultOffset = argPhFloor & ~BigInteger.One;
    ...
    private static SignedProperty? MakeSinPiProperty(BoundedRational arg)
    {
        ...
        if (nArg.HasValue)
        {
            ...
            if (nArg.HasValue) return new SignedProperty(MakeProperty(PropertyKind.IsSinPi, nArg), neg);
        }
    ...
    private static SignedProperty? MakeTanPiProperty(BoundedRational arg)
    {
        ...
        if (nArg.HasValue)
        {
            ...
            if (nArg.HasValue) return new SignedProperty(MakeProperty(PropertyKind.IsTanPi, nArg), neg);
        }
    ...
    {
        return CrFromProperty(p) ?? throw new ArgumentOutOfRangeException(nameof(p));
    }
    ...
    {
        var propertyCr = CrFromProperty(_crProperty);
        if (propertyCr == null) return true;

        var bound = MsbBound(_crProperty);
    ...
    {
        if (p == null || p.Kind != kind) return BoundedRational.Null;
    ...
    private static string SymbolicPiMultiple(BoundedRational r, bool degrees, bool unicodeFraction)
    {
        if (degrees)
        {
            var rInDegrees = r * Br180;
            if (rInDegrees.HasValue) return rInDegrees.ToDisplayString(unicodeFraction /* mixed fractions */);
    ...
    private static string? CrSymbolic(CRProperty? p, bool degrees, bool unicodeFraction)
    {
        ...
        var sqrtArg = GetSqrtArg(p);
        if (sqrtArg.HasValue)
        {
            var intSqrtArg = sqrtArg.ToBigInteger();
            return intSqrtArg != null
                ? $"{SqrtString}{intSqrtArg}"
                : $"{SqrtString}({sqrtArg.ToDisplayString(unicodeFraction)})";
        }

        var lnArg = GetLnArg(p);
        if (lnArg.HasValue) return $"ln({lnArg.ToDisplayString(unicodeFraction)})";

        var logArg = GetLogArg(p);
        if (logArg.HasValue) return $"log({logArg.ToDisplayString(unicodeFraction)})";

        var sinPiArg = GetSinPiArg(p);
        if (sinPiArg.HasValue) return $"sin({SymbolicPiMultiple(sinPiArg, degrees, unicodeFraction)})";

        var tanPiArg = GetTanPiArg(p);
        if (tanPiArg.HasValue) return $"tan({SymbolicPiMultiple(tanPiArg, degrees, unicodeFraction)})";

        var aSinArg = GetASinArg(p);
        if (aSinArg.HasValue)
            // sin superscript -1
            return
                $"sin{InverseString}({aSinArg.ToDisplayString(unicodeFraction)}){(degrees ? DegreeConversion : "")}";

        var aTanArg = GetATanArg(p);
        if (aTanArg.HasValue)
            return
    ...
    {
        return _crProperty != null && (IsOne(_crProperty) || _ratFactor.Sign == 0);
    }
    ...
    {
        if (DefinitelyRational() || _crProperty == null) return false;
    ...
    private static BoundedRational CommonPower(BigInteger x, BigInteger y)
    {
        ...
        if (compareResult == 0) return BoundedRational.One;

        if (compareResult < 0)
        {
            // BoundedRational already propagates null as desired.
            var inverse = CommonPower(y, x);
            return inverse.HasValue ? BoundedRational.Inverse(inverse) : BoundedRational.Null;
        }

        if (x.CompareTo(BigInteger.One) == 0 || y.CompareTo(BigInteger.One) == 0) return BoundedRational.Null;
        ...
        var recursiveResult = CommonPower(quotient, y);
    ...
    public static bool CommonPower(BoundedRational a, BoundedRational b)
    {
        ...
        {
            if (ndb.denominator.CompareTo(BigInteger.One) == 0) return CommonPower(numerator, ndb.numerator).HasValue;

            if (ndb.numerator.CompareTo(BigInteger.One) == 0) return CommonPower(numerator, ndb.denominator).HasValue;
        }
        else if (numerator.CompareTo(BigInteger.One) == 0)
        {
            if (ndb.numerator.CompareTo(BigInteger.One) == 0) return CommonPower(denominator, ndb.denominator).HasValue;

            if (ndb.denominator.CompareTo(BigInteger.One) == 0) return CommonPower(denominator, ndb.numerator).HasValue;
        }

        // In the general case, two commonPower computations must produce the same result.
        var commonPowerNaNb = CommonPower(numerator, ndb.numerator);
        var commonPowerNaDb = CommonPower(numerator, ndb.denominator);
    ...
    public bool DefinitelyIndependent(UnifiedReal u)
    {
        ...
        switch (p1.Kind)
        {
            ...
            case PropertyKind.IsSqrt:
                if (u.DefinitelyTranscendental()) return true;
            ...
            case PropertyKind.IsLn:
                if (p2.Kind == PropertyKind.IsIrrational) return false; // Not enough information.
    ...
    public string ToDisplayString(bool degrees, bool unicodeFraction, bool mixed)
    {
        ...
        var symbolic = CrSymbolic(_crProperty, degrees, unicodeFraction);
        if (symbolic != null)
        {
            var bi = _ratFactor.ToBigInteger();
            if (bi != null)
            {
                if (bi == BigInteger.One) return symbolic;

                if (bi == BigMinusOne) return $"-{symbolic}";

                return bi + symbolic;
            }

            var biInverse = BoundedRational.Inverse(_ratFactor).ToBigInteger();
            if (biInverse != null)
                // Use spaces to reduce ambiguity with square roots.
                return $"{(biInverse < 0 ? "-" : "")}{symbolic} / {BigInteger.Abs(biInverse.Value)}";

            if (unicodeFraction) return $"{_ratFactor.ToDisplayString(unicodeFraction /* mixed */)}{symbolic}";

            return $"({_ratFactor.ToDisplayString()}){symbolic}";
        }

        if (_ratFactor.Equals(BoundedRational.One)) return _crFactor.ToString();
    ...
    {
        return ToDisplayString(false, false, false);
    }
    ...
    {
        return _crProperty != null && _crProperty.DeterminesCr();
    }
    ...
    {
        var result = new ExprStats();
        var normalized = _ratFactor.Reduce();
        var bi = normalized.ToBigInteger();
        ...
        switch (_crProperty.Kind)
        {
            ...
            case PropertyKind.IsSqrt:
                ...
                var argAsBigInt = arg.ToBigInteger();
                var argBitLength = (int?)argAsBigInt?.GetBitLength() ?? arg.BitLength;
    ...
    {
        if (_crProperty != null && (IsOne(_crProperty) || _ratFactor.Sign == 0)) return _ratFactor.ToStringTruncated(n);

        var scaled = ConstructiveReal.FromBigInteger(BigInteger.Pow(10, n)) * ToConstructiveReal();
        var negative = false;
        ...
        if (ExactlyTruncatable())
        {
            ...
            if (ConstructiveReal.FromBigInteger(intScaled).CompareTo(scaled.Abs()) > 0) intScaled -= BigInteger.One;
    ...
    {
        return _crProperty != null && IsOne(_crProperty)
            ? _ratFactor.ToDouble() // Hopefully correctly rounded
            : ToConstructiveReal().DoubleValue();
        // Approximately correctly rounded
    }
    ...
    {
        return _ratFactor.CompareToOne() == 0 ? _crFactor : _ratFactor.ToConstructiveReal() * _crFactor;
    }
    ...
    {
        return _crProperty != null && u._crProperty != null &&
    ...
    public int CompareTo(UnifiedReal u)
    {
        ...
        if (SameCrFactor(u))
        {
            var sign = _crFactor.Sign(); // Can diverge if crFactor == 0.
        ...
        if (SameMonotonicCrKind(u))
        {
            ...
            if (_crProperty.Kind == PropertyKind.IsSqrt)
            {
                ...
                // size bounds.
                var sign = _ratFactor.Sign;
                var uSign = u._ratFactor.Sign;
                if (sign < uSign) return -1;

                if (sign > uSign) return 1;

                var sqrtArg = GetSqrtArg(_crProperty);
                var squared = BoundedRational.RawMultiply(
                    BoundedRational.RawMultiply(_ratFactor, _ratFactor), sqrtArg);
                var uSqrtArg = GetSqrtArg(u._crProperty);
                var uSquared = BoundedRational.RawMultiply(
    ...
    {
        if (IsComparable(u)) return CompareTo(u);

        // See if we can resolve comparison with lower precision first.
        for (var precision = DefaultInitialTolerance; precision * 2 > a; precision *= 2)
        {
            var result = ToConstructiveReal().CompareTo(u.ToConstructiveReal(), precision);
    ...
    /// </summary>
    public bool DefinitelyEquals(UnifiedReal u)
    {
    ...
    {
        if (_ratFactor.Sign == 0) return IsNonzero(u._crProperty) && u._ratFactor.Sign != 0;

        if (u._ratFactor.Sign == 0) return IsNonzero(_crProperty) && _ratFactor.Sign != 0;

        if (_crProperty == null || u._crProperty == null) return false;

        if (DefinitelyIndependent(u)) return _ratFactor.Sign != 0 || u._ratFactor.Sign != 0;

        if (SameCrFactor(u) && IsNonzero(_crProperty)) return !_ratFactor.Equals(u._ratFactor);
    ...
    {
        return _crProperty != null && IsOne(_crProperty) && _ratFactor.Equals(BoundedRational.One);
    }
    ...
    {
        return _crProperty != null && (IsOne(_crProperty) || _ratFactor.Sign == 0) ? _ratFactor : BoundedRational.Null;
    }
    ...
    private static UnifiedReal LogRep(PropertyKind kind, BoundedRational arg)
    {
        ...
        if (intArg != null)
        {
            var smallPowerLog = LgSmallPower(kind, intArg.Value);
            if (smallPowerLog != null) return smallPowerLog;
        }
    ...
    {
        if (x.Equals(y)) return new UnifiedReal(x);

        var product = (x * y).Reduce();
        if (product.Sign == 0) return Zero;
    ...
    {
        if (DefinitelyZero()) throw new DivideByZeroException("Inverse of zero");

        if (_crProperty != null && IsOne(_crProperty)) return new UnifiedReal(BoundedRational.Inverse(_ratFactor));

        var square = GetSqrtArg(_crProperty);
    ...
    {
        if (ApproxSign(DefaultComparisonTolerance) < 0) throw new ArithmeticException("sqrt(negative)");
        ...
        if (expArg.HasValue)
        {
            var newArg = expArg / BoundedRational.Two;
            if (newArg.HasValue) newCrProperty = MakeProperty(PropertyKind.IsExp, newArg);
        }
    ...
    private BigInteger? GetPiTwelfths()
    {
        ...
        {
            var quotient = (_ratFactor * BoundedRational.Twelve).ToBigInteger();
    ...
    {
        var piTwelfths = GetPiTwelfths();
        if (piTwelfths != null)
        {
            var result = SinPiTwelfths((int)piTwelfths.Value);
            if (result != null) return result;
        }

        if (_crProperty != null && IsPi(_crProperty))
        {
            var newCrProperty1 = MakeSinPiProperty(_ratFactor);
        ...
        var aSinArg = GetASinArg(_crProperty);
        if (aSinArg.HasValue && _ratFactor.CompareToOne() == 0) return new UnifiedReal(aSinArg);

        var newCrProperty = DefinitelyAlgebraic() && DefinitelyNonzero()
    ...
    {
        return arg._crProperty == null
            ? new UnifiedReal(arg._ratFactor, arg._crFactor,
                MakeProperty(PropertyKind.IsIrrational, BoundedRational.Null))
            : arg;
    }
    ...
    public UnifiedReal Tan()
    {
        ...
        if (piTwelfths != null)
        {
            var i = (int)piTwelfths.Value;
            if (i is 6 or 18) throw new DomainException("Tangent undefined");
        ...
        if (_crProperty != null && IsPi(_crProperty))
        {
            var newCrProperty1 = MakeTanPiProperty(_ratFactor);
            if (newCrProperty1 != null)
                return new UnifiedReal(
                    newCrProperty1.Negative ? BoundedRational.MinusOne : BoundedRational.One,
                    newCrProperty1.Property);
        ...
        var aTanArg = GetATanArg(_crProperty);
        if (aTanArg.HasValue && _ratFactor.CompareToOne() == 0) return new UnifiedReal(aTanArg);
    ...
    private static ConstructiveReal RecursivePow(ConstructiveReal @base, BigInteger exp)
    {
        ...
        var tmp = RecursivePow(@base, exp >> 1);
    ...
    private UnifiedReal ExpLnPow(BigInteger exp)
    {
        ...
        if (sign < 0)
        {
            var result = ((-ToConstructiveReal()).Ln() * ConstructiveReal.FromBigInteger(exp)).Exp();
            if ((exp & BigInteger.One) != BigInteger.Zero) /* odd exponent */ result = -result;
    ...
    private UnifiedReal Pow(BigInteger exp)
    {
        ...
        if (expSign == 0)
        {
            ...
            // zeroth power.
            if (DefinitelySign() != 0) return One;

            // Base is known to be exactly zero.
            throw new ZeroToTheZerothException("0^0");
        }

        if (DefinitelyZero() && expSign < 0) throw new ArithmeticException("zero to negative power");

        var absExp = BigInteger.Abs(exp);
        if (_crProperty != null && IsOne(_crProperty))
        {
            var resultLen = (double)exp * _ratFactor.ApproxLog2Abs();
            // Both multiplicands may be negative. That still implies a huge answer.
            if (resultLen > BitLimit /* +INFINITY qualifies */) throw new TooBigException("Power result is too big");

            if (absExp.CompareTo(HardRecursivePowLimit) <= 0)
            {
                var ratPow = _ratFactor.Pow(exp);
                // We count on this to fail, e.g. for very large exponents, when it would
                // otherwise be too expensive.
                if (ratPow.HasValue) return new UnifiedReal(ratPow);
            }
        }

        if (absExp.CompareTo(RecursivePowLimit) > 0) return ExpLnPow(exp);
    ...
    public UnifiedReal Pow(UnifiedReal exp)
    {
        if (_crProperty != null)
        {
            ...
            {
                if (_ratFactor.Equals(BoundedRational.One)) return exp.Exp();
            ...
            {
                var expLogArg = GetLogArg(exp._crProperty);
        ...
        var sign = ApproxSign(DefaultComparisonTolerance);
        var expAsBr = exp.ToBoundedRational();
        var knownIrrational = false;
        if (expAsBr.HasValue)
        {
            var expNumDen = expAsBr.NumDen;
            if (expNumDen.Item2 == BigInteger.One) return Pow(expNumDen.Item1);
            ...
            {
                var expDen = (int)expNumDen.Item2; // Doesn't lose information.
                // Don't just use BoundedRational.pow(), since that would bypass above checks.
                var rt = BoundedRational.NthRoot(_ratFactor, expDen);
                if (rt.HasValue) return new UnifiedReal(rt).Pow(expNumDen.Item1);
            ...
            if (expNumDen.Item2 == BigTwo) return Pow(expNumDen.Item1).Sqrt();
        }
        ...
        if (sign == 0 && DefinitelyZero())
        {
            ...
            var expSign = exp.DefinitelySign();
            if (expSign > 0) return Zero;

            if (expSign < 0) throw new ArithmeticException("zero to negative power");

            // Unclear we can get here.
            throw new ZeroToTheZerothException("0^0");
        }

        if (sign < 0) throw new ArithmeticException("Negative base for pow() with non-integer exponent");
    ...
    {
        var nAsDouble = (double)n;
        var approx = Math.Log(nAsDouble) / Math.Log(@base);
        ...
        if (double.IsInfinity(nAsDouble))
        {
            ...
            if (@base % 3 != 0 && n % BigThree == 0) return 0;

            if (@base % 5 != 0 && n % BigFive == 0) return 0;
        }
        ...
        long result = 0;
        var powers = new List<BigInteger> { new(@base) };
        var nReduced = n; // always equal to n/base^result .
        for (var i = 1;; ++i)
        {
            var last = powers[i - 1];
            var next = last * last; // base^(2^i)
            if (next.GetBitLength() > nReduced.GetBitLength()) break;

            var quotient = BigInteger.DivRem(nReduced, next, out var remainder);
            if (remainder != 0)
        ...
        // a power of base.
        for (var i = powers.Count - 1; nReduced != BigInteger.One; --i)
        {
            var power = powers[i];
            if (power.GetBitLength() <= nReduced.GetBitLength())
            {
                var quotient = BigInteger.DivRem(nReduced, power, out var remainder);
    ...
    {
        foreach (var m in SmallNonPowers)
        {
            var intLog = GetIntLog(n, m);
            if (intLog != 0)
            {
                ConstructiveReal newCrValue;
                if (kind == PropertyKind.IsLog)
                {
                    if (m == 10) return new UnifiedReal(new BoundedRational(intLog));
    ...
    public UnifiedReal Ln()
    {
        ...
        {
            if (sign == 0) throw new DomainException("log(zero)");

            var compare1 = CompareTo(One, DefaultComparisonTolerance);
            if (compare1 == 0)
            {
            ...
            if (bi != null)
            {
                if (_crProperty != null && IsOne(_crProperty))
                {
                    var smallPowerLn = LgSmallPower(PropertyKind.IsLn, bi.Value);
                    if (smallPowerLn != null) return smallPowerLn;
                }
                else
                {
                    // Check for n^k * sqrt(n), for which we can also return a more useful answer.
                    var square = GetSqrtArg(_crProperty).ToBigInteger();
                    if (square != null && square.Value.GetBitLength() < 30)
                    {
                        var intSquare = (int)square.Value;
                        var intLog = GetIntLog(bi.Value, intSquare);
                        if (intLog != 0)
                        {
                            var nRatFactor =
    ...
    public UnifiedReal Exp()
    {
        ...
        {
            var needSqrt = false;
            var ratExponent = _ratFactor;
            var asBigInt = ratExponent.ToBigInteger();
            if (asBigInt == null)
            {
            ...
            var nRatFactor = BoundedRational.Pow(lnArg, ratExponent);
            if (nRatFactor.HasValue)
            {
                var result = new UnifiedReal(nRatFactor);
                if (needSqrt) result = result.Sqrt();

                return result;
            }
        }

        if (CompareTo(BitLimitAsUReal, 0) > 0) throw new TooBigException("exp argument is too big");

        CRProperty? newCrProperty = null;
        if (_crProperty != null && IsOne(_crProperty)) newCrProperty = MakeProperty(PropertyKind.IsExp, _ratFactor);
    ...
    private static BigInteger GenFactorial(long n, long step)
    {
        if (n > 4 * step)
        {
            var prod1 = GenFactorial(n, 2 * step);
            var prod2 = GenFactorial(n - step, 2 * step);
        ...
        for (var i = n - step; i > 1; i -= step) res = res * new BigInteger(i);
    ...
    public UnifiedReal Fact()
    {
        ...
        if (asBigInt.Value.Sign < 0) throw new DomainException("Negative factorial argument");
        ...
        var biResult = GenFactorial((long)asBigInt.Value, 1);
        var nRatFactor = new BoundedRational(biResult);
    ...
    private static int MsbBound(CRProperty? p)
    {
        ...
        switch (p.Kind)
        {
            ...
            case PropertyKind.IsPi:
                return 1;
            case PropertyKind.IsSqrt:
                var wnb = p.Arg.WholeNumberBits;
                if (wnb == int.MinValue) return wnb;
            ...
            case PropertyKind.IsATan:
                ...
                if (p.Arg.CompareTo(Br1Over1024) >= 1) return -11;
    ...
    {
        if (_crProperty != null && (IsOne(_crProperty) || _ratFactor.Sign == 0)) return _ratFactor.DigitsRequired;
    ...
    {
        var crBound = MsbBound(_crProperty); // lower bound on binary log.
        if (crBound != int.MinValue)
        {
            var wholeBits = _ratFactor.WholeNumberBits;
            if (wholeBits == int.MinValue) return int.MaxValue;

            if (wholeBits + crBound >= 3) return 0;
    ...
    {
        Check(bound >= 0);
        var crBound = MsbBound(_crProperty);
        var ratBits = _ratFactor.WholeNumberBits;
        if (crBound != int.MinValue && ratBits != int.MinValue) return ratBits + crBound > bound;
